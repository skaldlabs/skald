"""
Simple Hello World LangChain Agent

To use this agent, you'll need to install:
...
#     return f"Hello, {name}! Welcome to the Skald chat agent!"
...
def create_chat_agent(streaming: bool = False, callback_handler=None):
    """Create and return a simple LangChain chat agent."""
...
async def async_stream_chat_agent(query: str, context: str = "") -> AsyncIterator[Dict[str, Any]]:
    """
    ...
    agent_executor = create_chat_agent(streaming=True, callback_handler=callback_handler)
    ...
    while True:
        try:
            ...
            event = await asyncio.wait_for(callback_handler.event_queue.get(), timeout=0.1)
