...
async def _process_rerank_batch_async(vc: voyageai.Client, query: str, batch: List[str]) -> List[Dict[str, Any]]:
    """
...
def foo(query: str) -> List[Dict[str, Any]]:
    ...
    rerank_data_batches = [rerank_data[i:i+25] for i in range(0, len(rerank_data), 25)]
...
async def prepare_context_for_chat_agent_async(query: str) -> List[Dict[str, Any]]:
    ...
    batch_tasks = [
        _process_rerank_batch_async(vc, query, batch) 
...
def prepare_context_for_chat_agent(query: str) -> List[Dict[str, Any]]:
    ...
    try:
        reranked_results = loop.run_until_complete(prepare_context_for_chat_agent_async(query))
