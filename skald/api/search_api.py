...
SUPPORTED_SEARCH_METHODS = ["title_contains", "title_startswith", "summary_vector_search", "chunk_vector_search"]
...
class SearchView(views.APIView):
    ...
    def post(self, request):
        ...
        if not search_method or search_method not in SUPPORTED_SEARCH_METHODS:
            return Response({"error": f"Search method is required and must be one of: {', '.join(SUPPORTED_SEARCH_METHODS)}"}, status=status.HTTP_400_BAD_REQUEST)
        
        if limit > 50:
            return Response({"error": "Limit must be less than or equal to 50"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        results = search_method_to_function[search_method](query, limit, tags)
        return Response({"results": results}, status=status.HTTP_200_OK)
        
        
    

def _summary_vector_search(query: str, limit: int, tags: list[str] = None):
    embedding_vector = generate_vector_embedding_for_search(query)
    memo_summary_results = memo_summary_vector_search(embedding_vector, limit, tags=tags)
    results = []
...
def _chunk_vector_search(query: str, limit: int, tags: list[str] = None):
    embedding_vector = generate_vector_embedding_for_search(query)
    memo_chunk_results = memo_chunk_vector_search(embedding_vector, limit, tags=tags)
    results = []
...
def _title_startswith_search(query: str, limit: int, tags: list[str] = None):
    memos = Memo.objects.all() if tags is None else Memo.objects.filter(memotag__tag__in=tags)
...
def _title_contains_search(query: str, limit: int, tags: list[str] = None):
    memos = Memo.objects.all() if tags is None else Memo.objects.filter(memotag__tag__in=tags)
