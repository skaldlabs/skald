...
function MythologyExplorer() {
    ...
    useEffect(() => {
        async function loadStory() {
            ...
            // Load characters
            const charData = await Promise.all(randomStory.characters.map((id) => client.mythology.getCharacter(id)))
...
program.name('odin-cli').description('CLI tool for Odin API').version('1.0.0')
