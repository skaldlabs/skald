...
async function vectorSearchNode(state: typeof RAGState.State) {
    ...
    const chunkResults = await memoChunkVectorSearch(project, embeddingVector, options.vectorSearch.topK, options.vectorSearch.similarityThreshold, filters)
...
async function rerankNode(state: typeof RAGState.State) {
    ...
    for (const chunkResult of chunkResults) {
        ...
        rerankMetadata.push({ memo_uuid: chunk.memo_uuid, memo_title: memoPropertiesMap?.get(chunk.memo_uuid)?.title || '' })
    }
    ...
    const results = (
        await Promise.all(
            rerankDataBatches.map((batch, idx) =>
                RerankService.rerank(searchQuery, batch, rerankMetadataBatches[idx])
            )
        )
    ).flat()
...
// async function* askLLMNode(state: typeof RAGState.State) {
//     const { query, conversationHistory, options, rerankedResults, clientSystemPrompt } = state;
...
//     const llm = LLMService.getLLM(0, options.llmProvider)
...
//     // TODO: should this be its own node?
//     if (options.references.enabled && rerankedResults.length > 0) {
